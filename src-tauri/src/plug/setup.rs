...
async fn update(app: tauri::AppHandle) -> tauri_plugin_updater::Result<()> {
    ...
    app.dialog()
        ...
        .title("NUCtool has a new version v".to_owned() + up.clone().version.as_str())
...
pub fn init(app: &mut App) -> Result<(), Box<dyn Error>> {
    ...
    println!("Autostart status: {}", autostart_manager.is_enabled()?);
    let config_dir = get_config_dir();
    fs::create_dir_all(&config_dir).unwrap();
    let config_beta = config_dir.join("beta.config");
    if !config_beta.exists() {
        println!("beta.config file not found");
    ...
    let window = app.get_webview_window("main").unwrap();
    let h = MenuItemBuilder::with_id("h", "Show Interface").build(app)?;
    let q = MenuItemBuilder::with_id("q", "Exit Program").build(app)?;
    let d = MenuItemBuilder::with_id("d", "Debug Mode").build(app)?;
    ...
    let _tray = TrayIconBuilder::new()
        ...
        .on_menu_event(move |app, event| match event.id().as_ref() {
            "h" => {
                println!("Show Interface clicked");
